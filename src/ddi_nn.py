...
def create_indexes(train_data, max_len=100):
    ...
    for sentences in train_data.values():
        for instances_of_a_sentence in sentences:
...
def encode_words_and_lemmas(split_data, indexes):
    ...
    for sentences in split_data.values():
        for instances_of_a_sentence in sentences:
...
def output_interactions(test_data, y_pred, out_file_path):
    ...
    index = 0
    for sid, pairs in test_data.items():
        for instance in pairs:
            id_e1, id_e2 = instance[0], instance[1]
            ddi_type = decode_idx_DDI(y_pred[index])
